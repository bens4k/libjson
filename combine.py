...
headers_path = os.path.join('src_multiple_files', 'include')
sources_path = os.path.join('src_multiple_files', 'src')
outfile = os.path.join('src_single_file', 'json.c')
...
outheader = ""
with open(os.path.join('src_multiple_files', 'include', 'json.h'), 'r') as f:
    contents = f.read()
    lines = contents.split('\n')
    lines = list(filter(lambda x: not x.startswith('#include'), lines))
    outheader = '\n'.join(map(str, lines))

with open('src_single_file/json.h', 'w') as fout:
