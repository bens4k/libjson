...
typedef enum errornum {ERR_UNEXPECTED_EOF, ERR_UNEXPECTED_SYM, 
					ERR_NO_MATCHING_BRACKET, ERR_KEY_MISSING,
					ERR_UNKNOWN_SYM, ERR_BLOCK_MISSING} ErrorNum;
...
// returns a cstring representation of the tokens in list
CString* tokens_list_cstring(const TokensList *list){
	Garbage* gr = new_garbage();
	Token *t = list->head;
...
// returns a cstring representation of given token t
CString* token_cstring(const Token *t){
	Garbage* gr = new_garbage();
	if(t != NULL){
		CString* cs = w(gr, new_cstring(0), GrCString);
...
// checks if c is a reserved char
char reserved_char(char c){
	return (c == '{') || (c == '}') || (c == '[') || (c == ']') || (c == 't') 
	|| (c == 'f') || (c == 'n') || (c == '\"') || (c == ':') || (c == ',') ||
	start_of_number(c) || (c == ' ') || (c == '\n') || (c == '\t'); 
}
// checks if cstring cs contains a valid number 
// conforming to the regex:  -?0|[1-9]\d*(\.\d+)?([eE][+-]?\d+)?
...
// looks for the next number contained in src_ch 
// and returns a cstring containing that number  
...
// create a token of type ty and value val
Token *new_token(TokenType ty, void *val, ulong line){
...
// allocate a linked list of tokens
TokensList *new_tokens_list(){
...
// checks if keyword exists in cs from location start
unsigned int matches(char* keyword, const CString* cs, uint start){
	Garbage* gr = new_garbage();
...
// returns a cstring containing the unknown block of text located at start
...
// returns a list of tokens extracted from csting ch
		...
		switch(ch->data[i]){
			...
			default:
			{
				if(start_of_number(ch->data[i])){// number
					...
					unsigned int l = matches("true", ch, i);
					if(l > 0){
					...
					unsigned int l = matches("false", ch, i);
					if(l > 0){
						i += l - 1; // 1 is added down
					...
					unsigned int l = matches("null", ch, i);
					if(l > 0){
						i += l - 1; // 1 is added down
...
// converts a tokens list back to a cstring format
CString *untokenize(const TokensList *list){
	Garbage *gr = new_garbage();
	// calc size of cstring
	ulong size = 0;
	Token *t = list->head;
	while(t != NULL){
